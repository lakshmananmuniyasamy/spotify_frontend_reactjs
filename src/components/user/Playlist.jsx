...
export const Playlist = () => {
    ...
    const fetchData = () => {
      axios.get("http://localhost:8080/file/getaddfav")
        .then((res) => {
      ...
      <div className='row'>
        {Array.isArray(posts) && posts.map((post) => (
          <div className="card m-2 col-12 col-sm-6 col-md-4 col-lg-3" key={post.id}>
            ...
            <img src={`http://localhost:8080/uploads/${post.artistImage}`} style={{ height: "180px", width: "100%" }} className="card-img-top" alt='songimage' />
            <div className='card-body'> 
              <ReactPlayer
                url={`http://localhost:8080/uploads/audio/${post.song}`}
                controls={true}
                width='100%'
                height='50px'
              />
